...
public class WriteTag extends BasicActivity {
    ...
    private void writeBlock() {
        ...
        //mDatabase.child( TagUID ).child("Saldo").setValue(  mDataText.getText() );
        ...
        mDatabase.child("Registro").child("ValorID").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    ...
                    int num = Integer.parseInt(mDataText.getText().toString(),16);

                    mDatabaseDOS.child( TagUID ).child("Saldo").setValue(  saldo );
