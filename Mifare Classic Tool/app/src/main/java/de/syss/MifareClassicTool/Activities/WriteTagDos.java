 ...
 */
public class WriteTagDos extends BasicActivity {
    ...
    private String mSectorTextBlock;
    private String mBlockTextBlock;
    ...
    public WriteTagDos(String Sector, String Bloque, String Data){
        this.mSectorTextBlock= Sector;
        this.mBlockTextBlock= Bloque;
        this.mDataText= Data;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //mB7lockTextBlock = findViewById(R.id.editTextWriteTagBlock);
                    ...
                    mDatabaseDOS.child( TagUID ).child("Saldo").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            if (!task.isSuccessful()) {
                                ...
                                EditText editTextWriteTagData = findViewById(R.id.editTextWriteTagData);

                                String str= "";
                                String strSaldo= "" + saldo;

                                for (int i = 0; i <= 32-strSaldo.length(); i++) {
                                    str+= "0";
                                }

                                editTextWriteTagData.setText( str + Integer.toHexString(saldo) );
    ...
    private void writeBlock() {
        ...
        int sector = Integer.parseInt(mSectorTextBlock);
        int block = Integer.parseInt(mBlockTextBlock);
        byte[][] keys = Common.getKeyMap().get(sector);
        int result = -1;

        if (keys[1] != null) {
            result = reader.writeBlock(sector, block,
                    Common.hexStringToByteArray(mDataText),
                    keys[1], true);
        }
    ...
    private void writeValueBlock() {
        ...
        int value = Integer.parseInt(mDataText);
        int sector = Integer.parseInt(mSectorTextBlock);
        int block = Integer.parseInt(mBlockTextBlock);
        byte[][] keys = Common.getKeyMap().get(sector);
        int result = -1;
