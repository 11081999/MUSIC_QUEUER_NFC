...
// ############################################################################
    ...
    public static int treatAsNewTag(Intent intent, Context context) {
        ...
        if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
            ...
            String TagUID= byte2HexString(tag.getId());

            final int[] visitas = new int[1];
            final int[] saldo = new int[1];

            DatabaseReference mDatabase = FirebaseDatabase.getInstance().getReference();
            DatabaseReference mDatabaseD = FirebaseDatabase.getInstance().getReference();
            mDatabase.child( "Registro" ).child("ValorID").setValue(TagUID);
            //mDatabase.child( TagUID ).child("Visitas").setValue(1);
            mDatabase.child( TagUID ).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        // TODO: handle the case where the data already exists

                        //int sal= snapshot.child("Saldo").getValue(int.class);
                        //int vis= snapshot.child("Vistas").getValue(int.class);

                        int number = 0;

                        switch(snapshot.child("Visitas").getValue(int.class)){
                            default:
                                number= number+400;
                        ...
                        mDatabaseD.child( TagUID ).child("Visitas").setValue( snapshot.child("Visitas").getValue(int.class) + 1);
                        mDatabase.child( TagUID ).child("Saldo"  ).setValue( snapshot.child("Saldo").getValue(int.class)  + number);
                    }
                    else {
            ...
            //updateDBvalues( byte2HexString(tag.getId()) );
    ...
    public static void updateDBvalues(String TagUID){
        ...
        //mDatabase.child(TagUID).child("Saldo").setValue(1);
        //mDatabase.child(TagUID).child("Visitas").setValue(2);
    }
