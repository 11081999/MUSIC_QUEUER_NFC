...
public class MainMenu extends Activity {
    ...
    private DatabaseReference mDatabaseDOS;
    private DatabaseReference mDatabaseREADNFC;

    private TextView textViewUID;
    private TextView textViewSaldo;
    private TextView textViewVisitas;

    private TextView editTextCancion;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDatabaseDOS = FirebaseDatabase.getInstance().getReference();
        mDatabaseREADNFC= FirebaseDatabase.getInstance().getReference();

        textViewUID= findViewById(R.id.textViewUID);
        textViewSaldo= findViewById(R.id.textViewSaldo);
        textViewVisitas= findViewById(R.id.textViewVisitas);

        editTextCancion= findViewById(R.id.editTextCancion);
...
/*
        mDatabase.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot ds:dataSnapshot.getChildren()){
                    ...
                    if(ds.getKey() == TagUID) {
                        visitas = ds.child("Visitas").getValue(int.class);

                        saldo = ds.child("Saldo").getValue(int.class);
                    }
    ...
    public void borrarDatos(View view) {
        ...
        //int result = (int)(randomNum * (10 - 1)) + 1;
        //mDatabase.child( "Registro" ).child("ValorID").setValue("0000" + result);

        mDatabase.child("Registro").child("ValorID").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    ...
                    TagUID= String.valueOf(task.getResult().getValue());

                    mDatabase.child( TagUID ).child("Saldo").setValue(0);
                    mDatabase.child( TagUID ).child("Visitas").setValue(0);
                }
            }
        });
        ...
        textViewUID.setText("UID: " + TagUID);
        textViewSaldo.setText("Saldo: " + saldo);
        textViewVisitas.setText("Visitas: " + visitas);
    }

    public void subirCancion(View view) {
        ...
        mDatabase.child("Registro").child("ValorID").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    ...
                    //mDatabase.child( TagUID ).child("queue").setValue(editTextCancion.getText().toString());

                    mDatabaseDOS.child( TagUID ).child("Saldo").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            if (!task.isSuccessful()) {
                                Log.e("firebase", "Error getting data", task.getException());
                                ...
                                saldo= Integer.parseInt(String.valueOf(task.getResult().getValue()));
                                ...
                                if(saldo <= 0){
                                    ...
                                    String text = "Saldo insufficiente";
                                    ...
                                    mDatabase.child( TagUID ).child("Saldo").setValue(saldo);
